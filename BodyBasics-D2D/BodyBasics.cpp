...
bool CBodyBasics::SetStatusMessage(WCHAR* szMessage, DWORD nShowTimeMsec, bool bForce)
{
...
void CBodyBasics::DrawHead(const D2D1_POINT_2F& headPosition, int bodycount, DetectionResult nResult)
{
	...
	StringCchPrintf(szStatusMessage, _countof(szStatusMessage),
		L"Body Index:%02d\nEngaged:(%d)%s", bodycount, nResult, szResult);

	m_pRenderTarget->DrawTextW(szStatusMessage, _countof(szStatusMessage), m_pTextFormat,
		D2D1::RectF(headPosition.x+50.0, headPosition.y, headPosition.x + 250.0, headPosition.y+100.0), m_pBrushHandOpen);
}
